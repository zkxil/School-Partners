...
const ExercisePublish: FC = () => {
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      topicList.forEach((topic: TopicList) => {
        topic.topicOptions = topic.topicOptions.map((option: string, id: number) => ({
          id: id + 1,
          option
        }))
      })
  ...
  const handleTopicTypeChange = (value: number, index: number) => {
    ...
    currentTopicList[index].topicType = value
    setTopicList(currentTopicList)
    form.setFieldsValue({ topicList: currentTopicList })
  }
    ...
    <div>
      ...
      <div className="exercise-publish__container">
        ...
        <Form form={form} layout="horizontal" {...formLayout} hideRequiredMark>
          {/* 题库名称 */}
          <Form.Item label="题库名称" name="exerciseName" rules={ExerciseNameRules}>
            <Input />
          </Form.Item>

          {/* 题库简介 */}
          <Form.Item label="题库简介" name="exerciseContent" rules={ExerciseContentRules}>
            <Input />
          </Form.Item>
          ...
          <Row>
            <Col span={12}>
              <Form.Item label="题库难度" labelCol={{ span: 8 }} wrapperCol={{ span: 10, offset: 2 }} name="exerciseDifficulty" rules={ExerciseDifficultyRules} initialValue={1}>
                <Select>
            ...
            <Col span={12}>
              <Form.Item label="题库类型" labelCol={{ span: 4 }} wrapperCol={{ span: 10, offset: 2 }} name="exerciseType" rules={ExerciseTypeRules} initialValue={1}>
                <Select>
          ...
          <Row>
            <Col span={12}>
              <Form.Item label="题库热门" labelCol={{ span: 8 }} wrapperCol={{ span: 10, offset: 2 }} name="isHot" valuePropName="checked" initialValue={false}>
                <Switch />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item label="是否公开" labelCol={{ span: 4 }} wrapperCol={{ span: 10, offset: 2 }} name="isPublic" valuePropName="checked" initialValue={false}>
                <Switch />
              </Form.Item>
            </Col>
          </Row>
          ...
          {topicList.map((topic, index) => (
            <Fragment key={index}>
              ...
              <Row gutter={32}>
                <Col span={12}>
                  <Form.Item style={{ display: 'none' }} name={['topicList', index, 'isUpload']}><span /></Form.Item>
                  <Form.Item label={<span>题目类型&nbsp;<Tooltip title="目前支持单选、多选及文件上传题"><InfoCircleOutlined /></Tooltip></span>} name={['topicList', index, 'topicType']} rules={TopicTypeRules} initialValue={1}>
                    <Select onChange={(value) => handleTopicTypeChange(value as number, index)}>
                ...
                <Col span={12} hidden={topic.isUpload}>
                  <Form.Item label="正确答案" name={['topicList', index, 'topicAnswer']} rules={topic.isUpload ? [{ required: false }] : TopicAnswerRules} initialValue={[1]}>
              ...
              <Row gutter={32} hidden={topic.isUpload}>
                {['A', 'B', 'C', 'D'].map((option, idx) => (
                  <Col span={12} key={idx}>
                    <Form.Item label={`选项${option}`} name={['topicList', index, 'topicOptions', idx]} rules={topic.isUpload ? [{ required: false }] : TopicOptionRules}>
                      <Input />
                    </Form.Item>
                  </Col>
                ))}
...
export default ExercisePublish
